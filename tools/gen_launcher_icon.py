...
⚠️ **いまは使っていない。** 配信中のアイコンは「なまえコール」時代の
   名刺カードの絵（`assets/images/launcher/*.png` にある完成データ）で、
   このスクリプトで作ったものではない。作り直したくなったときの
   たたき台として残してある。

⚠️ フリー素材の写真をアイコンにするのはやめた。ぱくたその規約は
   モデル写真の「イメージキャラクターや公式イメージとしての利用」を
   原則禁止していて、アプリアイコンはこれに当たる。承諾書が無いなら
   写真は使わないこと（`gen_launcher_icon_photo.py` は削除した）。
